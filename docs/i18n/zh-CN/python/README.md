...
| `aethermedia.metadata` | ID3 和 MP4 标签读写（通过 Mutagen）；NFO XML 抓取 |
| `aethermedia.playlist` | M3U 和 XSPF 播放列表解析与序列化 |
| `aethermedia.plugins` | 插件宿主——加载、激活并调用 VLC 风格的扩展脚本 |
| `aethermedia.cli` | 命令行入口点（`aether-media` 命令） |
