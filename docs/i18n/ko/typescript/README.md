...
| 모듈 | 내보내기 | 설명 |
|--------|--------|-------------|
| `player` | `AetherMediaPlayer` | HLS.js + Shaka Player 적응형 재생 엔진 |
| `social` | `FeedClient`, `ReactionClient` | 피드 탐색 및 반응 전송 |
| `streaming` | `AetherStreamClient` | 라이브 스트림 구독 및 세그먼트 버퍼링 |
| `content` | `ContentClient` | P2P 콘텐츠 청크 발견 및 다운로드 |
| `identity` | `ProfileClient` | AetherTag 프로필 조회 |
...
`AetherMediaPlayer`는 HLS.js(HTTP/HLS 소스용)와 Shaka Player(DASH 및 Aether 메시 스트림용)를 모두 래핑하여 최적의 엔진을 자동으로 선택합니다:
