...
| `aethermedia.metadata` | Leitura/escrita de tags ID3 e MP4 (via Mutagen); extração de XML NFO |
| `aethermedia.playlist` | Análise e serialização de playlists M3U e XSPF |
| `aethermedia.plugins` | Host de plugins — carregar, ativar e chamar scripts de extensão no estilo VLC |
| `aethermedia.cli` | Ponto de entrada da linha de comando (comando `aether-media`) |
