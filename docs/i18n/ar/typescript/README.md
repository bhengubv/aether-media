...
|--------|--------|-------------|
| `player` | `AetherMediaPlayer` | محرك التشغيل التكيّفي HLS.js + Shaka Player |
| `social` | `FeedClient`, `ReactionClient` | تصفح الخلاصة وإرسال التفاعلات |
| `streaming` | `AetherStreamClient` | اشتراك البث المباشر وتخزين الشرائح مؤقتاً |
| `content` | `ContentClient` | اكتشاف وتنزيل قطع المحتوى P2P |
| `identity` | `ProfileClient` | تحليل ملفات الشخصية بـ AetherTag |
...
يلفّ `AetherMediaPlayer` كلاً من HLS.js (لمصادر HTTP/HLS) وShaka Player (لتدفقات DASH وشبكة Aether)، ويختار المحرك الأفضل تلقائياً:
