...
| `aether_media.metadata` | ID3 および MP4 タグの読み書き（Mutagen 経由）; NFO XML スクレイピング |
| `aether_media.playlist` | M3U および XSPF プレイリストの解析とシリアライゼーション |
| `aether_media.plugins` | プラグインホスト — VLC スタイルの拡張スクリプトの読み込み・アクティベート・呼び出し |
| `aether_media.cli` | コマンドラインエントリポイント（`aether-media` コマンド） |
