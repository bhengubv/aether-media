...
| `aethermesh_media.metadata` | ID3 および MP4 タグの読み書き（Mutagen 経由）; NFO XML スクレイピング |
| `aethermesh_media.playlist` | M3U および XSPF プレイリストの解析とシリアライゼーション |
| `aethermesh_media.plugins` | プラグインホスト — VLC スタイルの拡張スクリプトの読み込み・アクティベート・呼び出し |
| `aethermesh_media.cli` | コマンドラインエントリポイント（`aether-media` コマンド） |
