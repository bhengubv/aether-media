...
`LruContentCache` in `Aether.Media.Content` provides an in-memory content cache with O(1) get, set, and eviction, backed by a doubly-linked list and a hash index.
