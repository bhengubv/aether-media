...
public static class ServiceCollectionExtensions
{
    ...
    /// A delegate that receives an <see cref="AetherMediaBuilder"/> and selects
    ...
    /// </code>
    /// </example>
    public static IServiceCollection AddAetherMedia(
        this IServiceCollection services,
        Action<AetherMediaBuilder> configure)
    {
        ArgumentNullException.ThrowIfNull(services);
        ArgumentNullException.ThrowIfNull(configure);

        var builder = new AetherMediaBuilder(services);
        configure(builder);
        return services;
    }
}
