...
/// Profile creation/update derives the node's AetherNetTag from its Ed25519 public key
...
public sealed class ProfileService : IProfileService
{
        ...
        CancellationToken ct = default)
    {
        ...
        var aethernetTag = AetherNetTag.FromPublicKey(_localPublicKey).Value;
        var profile = new MediaProfile(
            Uhid: _localUhid,
            DisplayName: displayName.Trim(),
            AvatarHash: null,
            Bio: bio?.Trim(),
            AetherNetTagValue: aethernetTag,
            FollowerCount: 0,
            FollowingCount: 0,
            ContentCount: 0,
            IsVerified: false,
        ...
        _logger.LogInformation("Created local profile {DisplayName} tag={Tag}", displayName, aethernetTag);
    ...
    /// <inheritdoc/>
    public Task<MediaProfile?> GetByAetherNetTagAsync(string aethernetTag, CancellationToken ct = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(aethernetTag);

        // Normalise to canonical XXXXX-XXXXX form before comparing.
        if (!AetherNetTag.TryParse(aethernetTag, out var parsed))
        ...
        {
            if (string.Equals(profile.AetherNetTagValue, canonicalTag, StringComparison.OrdinalIgnoreCase))
