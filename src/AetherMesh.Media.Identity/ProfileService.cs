...
/// Profile creation/update derives the node's AetherMeshTag from its Ed25519 public key
...
public sealed class ProfileService : IProfileService
{
        ...
        CancellationToken ct = default)
    {
        ...
        var aethermeshTag = AetherMeshTag.FromPublicKey(_localPublicKey).Value;
        var profile = new MediaProfile(
            Uhid: _localUhid,
            DisplayName: displayName.Trim(),
            AvatarHash: null,
            Bio: bio?.Trim(),
            AetherMeshTagValue: aethermeshTag,
            FollowerCount: 0,
            FollowingCount: 0,
            ContentCount: 0,
            IsVerified: false,
        ...
        _logger.LogInformation("Created local profile {DisplayName} tag={Tag}", displayName, aethermeshTag);
    ...
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(aethermeshTag);

        // Normalise to canonical XXXXX-XXXXX form before comparing.
        if (!AetherMeshTag.TryParse(aethermeshTag, out var parsed))
        ...
        {
            if (string.Equals(profile.AetherMeshTagValue, canonicalTag, StringComparison.OrdinalIgnoreCase))
