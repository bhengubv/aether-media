...
if TYPE_CHECKING:
    from aethermesh_media.models import MediaContent, MediaReaction, LiveStream
...
class PluginHost:
    """
    Maintains a registry of AetherMeshMediaPlugin instances and broadcasts
    ...
    def __init__(self) -> None:
        self._plugins: dict[str, AetherMeshMediaPlugin] = {}

    # ── Registration ──────────────────────────────────────────────────────

    def register(self, plugin: AetherMeshMediaPlugin) -> None:
        """Add a plugin to the registry.  Replaces any existing plugin with the same name."""
        if not isinstance(plugin, AetherMeshMediaPlugin):
            raise TypeError(f"Expected AetherMeshMediaPlugin, got {type(plugin).__name__}")
