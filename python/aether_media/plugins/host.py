...
class PluginHost:
    """
    Maintains a registry of AetherMediaPlugin instances and broadcasts
    ...
    def __init__(self) -> None:
        self._plugins: dict[str, AetherMediaPlugin] = {}

    # ── Registration ──────────────────────────────────────────────────────

    def register(self, plugin: AetherMediaPlugin) -> None:
        """Add a plugin to the registry.  Replaces any existing plugin with the same name."""
        if not isinstance(plugin, AetherMediaPlugin):
            raise TypeError(f"Expected AetherMediaPlugin, got {type(plugin).__name__}")
