"""Tests for aethernet_media.metadata.tag_reader."""
...
from aethernet_media.metadata.tag_reader import _first_str, _parse_track_number, read_tags
...
@patch("aethernet_media.metadata.tag_reader.os.path.exists", return_value=True)
@patch("aethernet_media.metadata.tag_reader.MutagenFile", return_value=None)
...
@patch("aethernet_media.metadata.tag_reader.os.path.exists", return_value=True)
@patch("aethernet_media.metadata.tag_reader.MutagenFile")
@patch("aethernet_media.metadata.tag_reader.isinstance")
...
def test_read_tags_mp4_with_cover(mock_mutagen_file, mock_exists):
    ...
    with patch("aethernet_media.metadata.tag_reader.MP4Cover", _FakeCover):
        mock_mutagen_file.return_value = audio
