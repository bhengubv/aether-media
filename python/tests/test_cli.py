"""Tests for aethernet_media.cli.__main__."""
...
from aethernet_media.cli.__main__ import (
    cmd_info,
    cmd_playlist,
    cmd_scan,
    main,
...
def test_cmd_scan_not_a_directory(capsys):
    with patch("aethernet_media.cli.__main__.os.path.isdir", return_value=False):
...
def test_cmd_info_file_not_found(capsys):
    with patch("aethernet_media.cli.__main__.os.path.isfile", return_value=False):
...
def test_cmd_info_success(tmp_path, capsys):
    ...
    with patch("aethernet_media.cli.__main__.os.path.isfile", return_value=True), \
         patch("aethernet_media.metadata.tag_reader.read_tags", return_value=fake_tags):
...
def test_cmd_info_read_tags_exception(tmp_path, capsys):
    ...
    with patch("aethernet_media.cli.__main__.os.path.isfile", return_value=True), \
         patch("aethernet_media.metadata.tag_reader.read_tags",
...
def test_cmd_info_mutagen_not_installed(tmp_path, capsys):
    ...
    with patch("aethernet_media.cli.__main__.os.path.isfile", return_value=True), \
         patch("aethernet_media.cli.__main__.read_tags" if False else
               "aethernet_media.metadata.tag_reader.read_tags",
               side_effect=ImportError("no module named mutagen")):
        # Simulate ImportError being raised on the import inside cmd_info
        with patch.dict("sys.modules", {"aethernet_media.metadata.tag_reader": None}):
            rc = cmd_info(str(fake_file))
...
def test_main_scan_dispatches(tmp_path):
    with patch("aethernet_media.cli.__main__.cmd_scan", return_value=0) as mock_scan, \
...
def test_main_info_dispatches(tmp_path):
    ...
    with patch("aethernet_media.cli.__main__.cmd_info", return_value=0) as mock_info, \
...
def test_main_playlist_dispatches(tmp_path):
    ...
    pl.write_bytes(b"x")
    with patch("aethernet_media.cli.__main__.cmd_playlist", return_value=0) as mock_pl, \
...
def test_main_scan_returns_exit_code_1(tmp_path):
    with patch("aethernet_media.cli.__main__.cmd_scan", return_value=1) as mock_scan, \
