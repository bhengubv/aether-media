...
|---|---|---|
| `ITransportService` | `Aether.Transport` | Sends encoded video/audio frames, reactions, and follow intents over the mesh (BLE / Wi-Fi Direct / NearLink / LoRa / HTTP relay) |
| `IStreamingService` | `Aether.Streaming` | Broadcasts `StreamAnnounce` when going live; `FeedAggregator` subscribes to `StreamAnnounced` and `StreamEnded` events to maintain the live-stream feed |
| `IContentService` | `Aether.Content` | Publishes `ContentDescriptor`s for uploaded media; `FeedAggregator` subscribes to `ContentAnnounced` for VOD discovery |
| `IDtnService` | `Aether.Dtn` | Delivers follow intents durably to offline creators; bundles wait up to 72 h for a route |
| `IMeshSender` | `Aether.Messaging` | Sends best-effort unfollow packets and live reactions over the mesh without DTN overhead |
| `IRoutingService` | `Aether.Routing` | Route-aware delivery of social packets; AODV-style RREQ/RREP with Ed25519-signed route replies |
| `SignalProtocolService` | `Aether.Security` | End-to-end encrypts direct messages, profile sync payloads, and private channel content with X3DH + Double Ratchet |
| `IAdaptiveBitrateController` | `Aether.Streaming` | Selects the highest sustainable quality rung (H.264 / H.265 / VP8) based on live bandwidth estimates from the active transport |
