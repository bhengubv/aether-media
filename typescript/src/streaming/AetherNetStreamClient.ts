import type { AetherNetMediaPlayer } from "../player/AetherNetMediaPlayer.js";

type SegmentCallback = (bytes: Uint8Array) => void;

/**
 * Subscribes to an Aether mesh stream endpoint and feeds MPEG-TS or fMP4
 * segments to an AetherNetMediaPlayer via its MediaSource API.
 *
 ...
 *
 * Usage:
 *   const client = new AetherNetStreamClient(player);
 *   await client.subscribe("stream://aether/abc123");
 *   // later:
 *   client.unsubscribe();
 */
export class AetherNetStreamClient {
  private readonly player: AetherNetMediaPlayer;
  ...
  constructor(player: AetherNetMediaPlayer) {
    this.player = player;
  }
  ...
  async subscribe(streamId: string): Promise<void> {
    ...
    const url = streamId.startsWith("aether://")
      ? `https://relay.aethernet.network/stream/${streamId.slice("aether://".length)}`
      : streamId;
    ...
    try {
      ...
      while (true) {
        ...
        while (buffer.length >= 4) {
          ...
          this.player.feedSegment(segment, AetherNetStreamClient.MIME_CODEC);
        }
      }
    } catch (err) {
      if (signal.aborted) return; // normal cancellation
      console.error("[AetherNetStreamClient] stream error:", err);
    } finally {
