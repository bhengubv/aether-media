 ...
 * segments to an AetherMeshMediaPlayer via its MediaSource API.
 *
 ...
 *   client.unsubscribe();
 */
export class AetherMeshStreamClient {
  private readonly player: AetherMeshMediaPlayer;
  ...
  constructor(player: AetherMeshMediaPlayer) {
    this.player = player;
  }
  ...
  async subscribe(streamId: string): Promise<void> {
    ...
    const url = streamId.startsWith("aether://")
      ? `https://relay.aethermesh.network/stream/${streamId.slice("aether://".length)}`
      : streamId;
    ...
    try {
      ...
      while (true) {
        ...
        while (buffer.length >= 4) {
          ...
          this.player.feedSegment(segment, AetherMeshStreamClient.MIME_CODEC);
        }
      }
    } catch (err) {
      if (signal.aborted) return; // normal cancellation
      console.error("[AetherMeshStreamClient] stream error:", err);
    } finally {
