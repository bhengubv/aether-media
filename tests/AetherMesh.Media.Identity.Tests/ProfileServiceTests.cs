...
using AetherMesh.Media.Identity.Tests.Helpers;

namespace AetherMesh.Media.Identity.Tests;
...
public sealed class ProfileServiceTests
{
    ...
    // ── GetByAetherMeshTagAsync ────────────────────────────────────────────────

    [Fact]
    public async Task GetByAetherMeshTagAsync_InvalidTag_ReturnsNull()
    {
        var service = new ProfileService(new FakeMeshSender(), TestKey);
        var profile = await service.GetByAetherMeshTagAsync("not-a-valid-tag");
        Assert.Null(profile);
    }
}
